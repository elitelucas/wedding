...
class Crud_model extends CI_Model
{
    ...
    ///multi where///
    function multi_where($table_name,$first_type, $first_value, $second_type, $second_value,$field){
     
        if($first_type!='' && $first_value!='' && $second_type!='' && $second_value!=''){
            $array = array($first_type => $first_value, $second_type => $second_value);
            $l = $this->db->get_where($table_name, $array);
            $n = $l->num_rows();
            if ($n > 0) {
                return $l->row()->$field;
            }
