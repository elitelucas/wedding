...
class Home extends CI_Controller {
  ...
  function instamojo_success(){
    ...
    function registration($para1="")
    {
        ...
        if ($this->member_permission() == TRUE) {
            ...
            elseif ($para1=="sumit_profile_details") {
                ...
                $basic_info = $this->Crud_model->get_type_name_by_id('member', $ses_user_data, 'basic_info');
                ...
                $basic_info_tmp[] = array('age'               => '',
                                      ...
                                      'number_of_children'    => '',
                                      'area'                  => '',
                                      'on_behalf'             => $basic_info_data[0]["on_behalf"]
                                     );
                ...
                $nData = array(
                    ...
                    'religion' => $this->input->post('religion'),
                    'caste' => $this->input->post('caste'),
                    'subcaste' => $this->input->post('sub_caste'),
            ...
            elseif ($para1=="career_details") {
                ...
                $height_degree = $this->db->get_where('degree', array())->result();
                ...
                $page_data['height_degree'] = $height_degree;
            ...
            elseif ($para1=="sumit_career_details") {
                ...
                $this->db->where('member_id', $ses_user_data);
                $this->db->update('member', $nData);
                redirect(base_url().'home/registration/contact_details', 'refresh');
            }
             elseif ($para1=="contact_details") {
                if ($this->Crud_model->get_settings_value('third_party_settings', 'captcha_status', 'value') == 'ok') {
                    $page_data['recaptcha_html'] = $this->recaptcha->render();
                }
                ...
                $residence = $this->db->get_where('residence', array())->result();


                $page_data['countries'] = $countries;
                $page_data['states'] = $states;
                $page_data['cities'] = $cities;
                $page_data['residence'] = $residence;
              ...
              elseif ($para1=="sumit_contact_details") {
                $ses_user_data = $this->session->userdata('pending_reg_user');
            ...
            elseif ($para1=="lifestyle_family") {
                ...
                $family_value = $this->db->get_where('family_value', array())->result();
                $family_status = $this->db->get_where('family_status', array())->result();
         
                $brothers_sister = $this->db->get_where('brothers_sister_count', array())->result();
                $mother_tounge = $this->db->get_where('mother_tounge', array())->result();
                
                $page_data['family_value'] = $family_value;
                $page_data['family_types'] = $family_types;
                $page_data['family_status'] = $family_status;
              
                $page_data['brothers'] = $brothers_sister;
                $page_data['sisters'] = $brothers_sister;
                $page_data['mother_tounge'] = $mother_tounge;
            ...
            elseif ($para1=="sumit_lifestyle_family") {
                ...
                $nData = array(
                    ...
                    'brothers_married' => $this->input->post('brothers_married'),
                    'sisters_married' => $this->input->post('sisters_married'),
                    'mother_tounge' => $this->input->post('mother_tounge'),
                    'relative_caste' => $this->input->post('relative_caste'),
    ...
    function get_dropdown_by_id_city($table,$field,$id,$city="")
    {
        $options = $this->db->get_where($table, array($field=>$id))->result();
        $table_id = $table."_id";
        $table_name = "name";
